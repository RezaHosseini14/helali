...
function AudiosBox({ audios }: { audios: audioTypes[] }) {
  ...
  return (
    <Panel title="صوت" fill icon="ki-solid ki-abstract-45">
      <div className="grid grid-cols-12 gap-4">
        {audios?.length > 0 ? (
          audios?.slice(-12)?.map((audio: audioTypes) => (
            <button
              className="group xl:col-span-3 lg:col-span-4 sm:col-span-6 col-span-12 flex items-center justify-between border border-mainColor rounded-xl p-2 bg-mainColor/10 backdrop-blur-lg"
              onClick={() => {
            ...
            >
              <div className="flex items-end gap-2">
                ...
                <div className="flex flex-col flex-1 gap-2 h-full justify-end">
                  <span className="font-bold w-32 truncate text-right">{audio?.title}</span>
